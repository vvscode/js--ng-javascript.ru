angular.module('ngMail', [])
                    ...
                    <div class="col-sm-3 col-md-2">
                        <div class="btn-group">
                            ...
                            <ul class="dropdown-menu" role="menu">
                                <li ng-repeat="item in list"><a href="{{item.link}}">{{item.title}}</a></li>
                            </ul>
                        </div>
                    </div>`
        }
        ...
        return {
            ...
            template: `
                <ul class="nav nav-pills nav-stacked">
                    <li ng-repeat="item in list" class="{{item.active? 'active': ''}}">
        ...
        return {
            ...
            template: `
                <ul class="nav nav-tabs">
                    <li ng-repeat="item in list" class="{{item.active? 'active': ''}}"><a href="#{{item.id}}" data-toggle="tab"><span class="glyphicon {{item.icon}}"></span>{{item.title}}</a></li>
                </ul>`
        }
    })
    ...
    .controller('PageController', function() {
        ...
        this.mailListItems = [1, 2, 3, 4, 5, 6, 7].map((item) => ({
